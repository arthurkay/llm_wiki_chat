...
export function searchWiki(query: string, limit = 5): WikiHit[] {
	...
	if (!match) {
		const rows = db
			.prepare('SELECT path, title, body FROM wiki_pages ORDER BY updated_at DESC LIMIT ?')
	...
	try {
		const rows = db
			.prepare(
				`SELECT f.path AS path, f.title AS title, p.body AS body, rank AS rank
...
export function expandWikilinks(hits: WikiHit[], perHit = 2): WikiHit[] {
	...
	for (const h of hits) {
		...
		while ((m = WIKILINK.exec(h.body)) !== null && n < perHit) {
			...
			const row = db
				.prepare(
					'SELECT path, title, body FROM wiki_pages WHERE title = ? OR path LIKE ? LIMIT 1'
				)
